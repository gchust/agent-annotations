      ...
      const rawUrl = typeof input === "string"
        ...
        : input instanceof URL
          ...
          : input instanceof Request
            ? input.url
            : "";
      // Derive the method from Request.method when init.method is absent.
...
const subscribeNetworkFailures = (listener: NetworkFailureListener): (() => void) => {
...
export async function mountAgentAnnotations(
  ...
  const sanitizeNetworkUrl = (raw: string): string => {
    try {
      // Resolve relative requests against the current page so recorded URLs
      // are always an absolute http(s) origin+path; other schemes (mailto,
      // file, data, ...) are rejected like the server boundary does.
  ...
  const isOwnEndpoint = (raw: string): boolean => {
    ...
    try {
      const endpoint = new URL(browserStatus.endpoint, window.location.href);
      const target = new URL(raw, window.location.href);
      // Same origin plus the exact endpoint path (or a descendant of it).
  ...
  const recordNetwork = (
    ...
    // The snapshot is a public data boundary: entries must already match the
    // server-accepted bounds (method 1..16 uppercase A-Z, origin+path <= 2000)
    // or they are skipped here, never relying on server rejection.
  ...
  // Subscribe while mounted; the subscription is ref-counted and restored
  // identity-safely when the last live mount unsubscribes.
  const unsubscribeNetworkDiagnostics = diagnosticsConfig.network === false
    ? null
    : subscribeNetworkFailures((failure) => {
