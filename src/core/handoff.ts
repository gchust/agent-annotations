...
export function formatAgentAnnotationsHandoff(
  ...
  const lines = [
    ...
    "## Instructions",
    "",
    "- Use the annotations below as the work list and modify the real application source code; editing active-task.json is not a solution.",
    `- As soon as the relevant source is modified, write a UTF-8 implementation summary and run the exact ${command} complete command below; if it reports annotation not found, continue without retrying.`,
    "- Keep verification fast: run only the smallest check relevant to the changed files; do not run the full test suite unless explicitly requested.",
